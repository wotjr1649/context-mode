...
|---|---|
| D1 | 첫 문서는 **기반**만 |
| D2 | **전면 삭제** (212파일 = 단계 1에서 8 + 단계 3에서 204) |
...
| 1 | **CI 무해화 + 봇 정지** (§7). 파일 8개 삭제 | 워크플로에서 상류 호출 grep 0건, 자동 커밋 봇 0건 |
...
9. **스크립트·매니페스트 수정(D5):** `scripts/version-sync.mjs`의 `TARGETS`에서 삭제된 7개 경로 제거 · `package.json`의 `version` 스크립트 `git add` 목록 동기화 · **`package.json`의 `files` 필드에서 `.openclaw-plugin`·`openclaw.plugin.json` 제거(F32)** · `postinstall.mjs`의 OpenClaw 감지 블록 제거 · `package.json`의 `install:openclaw` 제거(삭제된 `.sh` 참조) · `scripts/ctx-debug.sh` 정리.
