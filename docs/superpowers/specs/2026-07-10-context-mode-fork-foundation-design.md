...
9b. **`pluginKey` 파생 수정 잔여분(F42·F43·F52, D11의 이월분).** `start.mjs`의 네 지점은 **단계 2에서 이미 고쳤다**(F53). 여기서 남은 것:
   ...
   - `start.mjs:348`의 `healScript` 템플릿(배포되는 `cache-heal.mjs`의 원본)
   - **`heal-installed-plugins.mjs:547-557`의 `sweepStaleMcpJson` 키→경로 매핑 반전(F54).** 고치지 않으면 이 자가치유는 개명 후에도 계속 죽는다
