...
### Task 4b: 사용자 레벨 힐 훅을 새 마켓플레이스로 마이그레이션

**Files:** `C:\Users\js\.claude\hooks\context-mode-deps-heal.mjs`, `C:\Users\js\.claude\hooks\context-mode-lock-heal.mjs`

**왜 필요한가:** 두 훅은 `installed_plugins.json`의 키 `"context-mode@context-mode"`와 캐시 부모 `cache/context-mode/context-mode`를 하드코딩한다(F50). 개명 후 **옛 트리를 치유하고 새 트리는 방치한다.** `deps-heal`은 `@mixmark-io/domino` 부분 설치를 고치려고 쓴 훅인데, 엉뚱한 `node_modules`를 고치게 된다 — 안전망 상실이 아니라 **기능 상실**이다.

`cache-heal`은 손대지 않는다 (`start.mjs`가 매 부팅 덮어쓴다). `prune-versions`는 옛 키 항목이 0이면 fail-safe `exit(0)`이라 무해하다. `orphan-reaper`는 키를 안 쓴다.

- [ ] **Step 1: 현재 하드코딩 확인 (RED)**

```bash
grep -n 'context-mode@context-mode' /c/Users/js/.claude/hooks/context-mode-deps-heal.mjs
grep -n 'cache", *"context-mode", *"context-mode"' /c/Users/js/.claude/hooks/context-mode-deps-heal.mjs /c/Users/js/.claude/hooks/context-mode-lock-heal.mjs
```
...
이유: 개명 후에도 `start.mjs:253`이 리터럴 `"context-mode@context-mode"`를 `healSettingsEnabledPlugins`에 넘긴다. 그 함수는 `current === false`일 때만 `explicit-opt-out`으로 건너뛰고, `undefined`면 **`true`로 되살린다**(F43). 키를 지우면 매 부팅 유령 플러그인이 다시 활성화된다.

(`postinstall.mjs`의 같은 리터럴 6개는 `if (isGlobalInstall())`로 게이트돼 있어 `/plugin install` 경로에서는 실행되지 않는다 — F44.)

Claude는 이 `false`를 덮어쓰지 않는다. 자동 키 재작성은 마켓플레이스의 `renames` 맵(**플러그인** 개명)에만 발동한다(F47).
...
> **경로 구분자에 주의.** Claude Code는 MCP 서버를 **포워드슬래시** 경로로 띄운다. 실측(PID 40832):
> `"C:/Program Files/nodejs/node.exe" C:/Users/js/.claude/plugins/cache/context-mode/context-mode/1.0.169/start.mjs`
> 백슬래시 전용 정규식은 `NO-MATCH`를 내고 **모든 프로세스를 "캐시 밖"으로 오판해 거짓 PASS**를 만든다. 아래 정규식은 `[\\/]`로 양쪽을 받는다.

```powershell
$rx = '(?i)cache[\\/](context-mode[^\\/]*)[\\/]context-mode[\\/]([\d.]+)'
Get-CimInstance Win32_Process |
  Where-Object { $_.ProcessId -ne $PID -and $_.CommandLine -match $rx } |
  ForEach-Object {
    $null = $_.CommandLine -match $rx
    [pscustomobject]@{ PID = $_.ProcessId; Tree = "$($Matches[1])/$($Matches[2])" }
  } | Format-Table -AutoSize
```
Expected: 출력되는 모든 행이 `context-mode-js/1.0.0`. `context-mode/1.0.169`가 하나라도 보이면 **실패**. 행이 하나도 없으면 MCP 서버가 안 뜬 것이므로 역시 실패다.

`-ne $PID`로 자기 자신을 제외한다 — 이 명령의 CommandLine 자체가 `context-mode` 문자열을 포함한다.
...
```powershell
$rx = '(?i)cache[\\/]context-mode-js[\\/]context-mode[\\/]'
$procs = Get-CimInstance Win32_Process
$alive = $procs.ProcessId
($procs | Where-Object {
   $_.ProcessId -ne $PID -and $_.CommandLine -match $rx -and ($alive -notcontains $_.ParentProcessId)
 }).Count
```
Expected: `0`

`-like "*<백슬래시 경로>*"`를 쓰면 안 된다 — CommandLine이 포워드슬래시라 절대 매칭되지 않고 항상 `0`을 반환해 거짓 통과한다.
...
**첫 줄이 `false`를 되돌리는 것이어야 한다.** Step 7에서 넣은 `"context-mode@context-mode": false`를 그대로 두면 `healSettingsEnabledPlugins`가 `explicit-opt-out`으로 존중해 **아무도 다시 활성화하지 않는다.** 완화책이 만든 함정이다.

```
1. ~/.claude/settings.json 의 enabledPlugins["context-mode@context-mode"] 를 true 로 되돌린다
2. Task 4b 에서 고친 deps-heal / lock-heal 을 되돌린다 (git -C ~/.claude revert)
...
**설치되는 것은 `1.0.169`가 아니라 상류의 최신 버전이다.** 로컬 캐시에 옛 dir이 남아 있어도 설치 버전을 고정하지 못한다. 특정 버전이 필요하면 `source.ref`로 태그를 고정해야 한다.

**롤백 창은 약 7일이다** — 고아 캐시 디렉토리는 7일 후 자동 삭제된다(F48).
