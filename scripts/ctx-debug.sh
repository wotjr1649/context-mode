...
# ─── 3. ctxscribe Installation ────────────────────────────────────────────

section "3. ctxscribe Installation"
...
# npm latest
NPM_LATEST="$(safe_cmd_quiet npm view ctxscribe version 2>/dev/null)"
...
# Detection logic (mirrors ctxscribe adapter selection — hard fork:
...
FTS5_RESULT="$(safe_cmd_quiet node -e "
  try {
    ...
    db.exec(\"INSERT INTO t VALUES('ctxscribe diagnostic test')\");
...
if command -v ps &>/dev/null; then
  CTX_PROCS="$(ps aux 2>/dev/null | grep '[c]ontext-mode' | grep -v -E 'ctxscribe-dashboard|esbuild|wrangler|workerd|grep' || true)"
...
kv "Running ctxscribe processes" "$CTX_COUNT"
...
if [ "$CTX_COUNT" -gt 3 ]; then
  warn "Multiple ctxscribe processes — possible orphans"
fi
...
printf '\n'
printf '  ctxscribe diagnostic v%s\n' "$CTX_DEBUG_VERSION"
printf '  ─────────────────────────────\n'
